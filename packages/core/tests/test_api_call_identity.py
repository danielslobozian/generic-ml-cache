...
def test_different_context_fingerprints_differ():
    assert (
        _identity(context_fingerprint="a").generate_key()
        != _identity(context_fingerprint="b").generate_key()
    )


def test_different_prompt_fingerprints_differ():
    assert (
        _identity(prompt_fingerprint="a").generate_key()
        != _identity(prompt_fingerprint="b").generate_key()
    )


def test_system_fingerprint_affects_key():
    without = _identity(system_fingerprint=None).generate_key()
    with_sys = _identity(system_fingerprint="sfp").generate_key()
    assert without != with_sys
...
def test_never_collides_with_a_passthrough_key():
    api_key = _identity(provider="claude", prompt_fingerprint="x").generate_key()
