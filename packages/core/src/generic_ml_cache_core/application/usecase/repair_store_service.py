...
class RepairStoreService(RepairStoreUseCase):
    ...
    def __init__(
        ...
        artifact_persistence: ArtifactPersistenceRepairPort,
        ...
        for run in self._repair_source.runs_awaiting_persistence():
            ...
            for blob_key in run.blob_keys:
                if self._blob_store.get(blob_key) is not None:
                    self._artifact_persistence.mark_artifacts_stored(run.execution_id, blob_key)
                    blobs_reconciled += 1
                else:
                    self._artifact_persistence.mark_artifacts_failed(
                        run.execution_id, blob_key, "blob missing on repair"
                    )
                    blobs_missing += 1
                    all_present = False
            if all_present:
                self._artifact_persistence.finalize_output_persisted(run.execution_id)
                runs_recovered += 1
            else:
