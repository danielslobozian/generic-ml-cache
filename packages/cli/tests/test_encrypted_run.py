...
def test_encrypted_run_stores_ciphertext_and_replays_with_token(tmp_path):
    ...
    execution = build_use_cases(store, encryption_token=token, client="fake").run_ml.execute(
        _command()
    )
    ...
    served = build_use_cases(store, encryption_token=token, client="fake").run_ml.execute(
        _command()
    )
...
def test_public_store_keeps_plaintext_and_ignores_a_token(tmp_path):
    ...
    execution = build_use_cases(store, encryption_token="ignored", client="fake").run_ml.execute(
        _command()
    )
    assert execution.execution_state is ExecutionState.SUCCESS
...
def test_encrypted_store_without_token_blocks_content_but_not_metadata(tmp_path):
    ...
    token = AesGcmCipher().generate_token()
    _encrypt_store(store, token)
    recorded = build_use_cases(store, encryption_token=token, client="fake").run_ml.execute(
        _command()
    )
