...
- `1.0.0` is the **stable, feature-rich** release: scope tokens, sessions, reporting,
  asynchronous executions, alias mode, API adapters, and scope-aware retention have all
  landed, and the CLI surface, execution-record schema, and adapter contract are locked
...
Schema-shaping features (scopes, sessions) deliberately land **before** `1.0.0`, while
`0.x` still permits the record schema to change — so `1.0.0` can lock a schema that is
already scope- and session-aware rather than promise stability it would soon have to break.
...
### 0.5.0 — Session and scope reporting
...
### 0.9.0 — Scope-aware retention and invalidation
...
- Stable execution-record schema (scope- and session-aware) and compatibility policy.
