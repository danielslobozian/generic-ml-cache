...
- Session IDs are generated by gmlcache; a session needs only an id (no token, no account).
- A session is an append-only log of invocation events; it supports workflow-level reports.
