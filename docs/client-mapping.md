...
| Web / network access | `--grant net` (optional; keyed, stays cacheable) | `--dangerously-skip-permissions` (replaces the write door for a net call; the narrow tool-allow was flaky ~2/3, this is 3/3) — verified | network on in the write-sandbox (`-c sandbox_workspace_write.network_access=true`) — verified | `--force` (its `--trust` write door alone does not open the sandboxed network) — verified |
