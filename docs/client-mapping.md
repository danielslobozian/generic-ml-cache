...
| Web / network access | `--grant net` (optional; keyed, stays cacheable) | web tools allowed (`--allowedTools WebSearch WebFetch`) — verified | network on in the write-sandbox (`-c sandbox_workspace_write.network_access=true`) — verified | `--force` (its `--trust` write door alone does not open the sandboxed network) — verified |
