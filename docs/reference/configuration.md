...
| `adapters` | comma-separated plugin names (or `*`) | *bundled only* | *(none)* | Opts **third-party** adapter plugins into loading, by entry-point name. The bundled adapters always load; a third-party plugin loads only if named here, so an untrusted installed plugin never imports on its own. Omit (or `*`) to run with the bundled adapters only. |
